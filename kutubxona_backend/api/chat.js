...
- Qisqa, aniq va foydali bo'lsin`;
    ...
    try {
        ...
        if (useGeminiFirst && geminiKey) {
            try { reply = await tryGemini(); } catch (e) {
                if (groqKey) { try { reply = await tryGroq(); } catch (e2) { } }
            }
        } else if (groqKey) {
            try { reply = await tryGroq(); } catch (e) {
                if (geminiKey) { try { reply = await tryGemini(); } catch (e2) { } }
            }
        } else if (geminiKey) {
            try { reply = await tryGemini(); } catch (e) { }
        }
